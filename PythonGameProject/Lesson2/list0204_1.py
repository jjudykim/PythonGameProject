...
def main():
    ...
    timer = timer + 1
    canvas.delete("TIMER")
    canvas.create_text(50, 50, text="timer " + str(timer), fill="yellow", font=fnt1, tag="TIMER")
    canvas.create_text(200, 30, text="index " + str(index), fill="white", font=fnt1, tag="STATUS")
    canvas.create_text(400, 30, text="index " + str(index), fill="white", font=fnt1, tag="STATUS")
    if index == 0:
        if timer == 1:
            canvas.create_text(300, 150, text="타이틀", fill="white", font=fnt2, tag="TITLE")
            canvas.create_text(300, 300, text="Press [SPACE]", fill="lime", font=fnt1, tag="TITLE")
        if key == "space":
            canvas.delete("TITLE")
